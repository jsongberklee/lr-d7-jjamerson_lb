<?php if(isset($_GET['notemplate']) && $_GET['notemplate'] == 1): ?>
	<div class="dataset" id="event_dataset_BeanTown2012">
		<h3>BeanTown Jazz Festival 2012</h3>
		<ul>
			<?php
				foreach($view->result as $vitem):
					$item = $vitem->_field_data['nid']['entity'];
					$venue = node_load($item->field_event_venue['und'][0]['target_id']);
					...
					//$start_date   = date('l, F j, Y, g:i a',strtotime($item->field_dates['und'][0]['value'].' - 4 hours'));
					$teaser       = $item->field_teaser['und'][0]['value'];
					$location     = $venue->title; //$item->field_location['und'][0]['value'];
